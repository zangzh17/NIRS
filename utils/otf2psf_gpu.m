function psf = otf2psf_gpu(varargin)
...
% Class Support
% -------------
% OTF can be any nonsparse, numeric array or gpuArray. PSF is of the same class as OTF.
%
% Example
% -------
...
if ~isnumeric(otf) || issparse(otf)
    ...
    % Convert to double while preserving array type (GPU or CPU)
    if ~isa(otf, 'double')
        otf = double(otf);
    end
